...
foreach ($Log in $EventVwr) {"{0,-28} {1,-20} {2,-8}" -f $Log.log, $Log.OverflowAction, $Log.MaximumKilobytes}
...
foreach($t in $r){"{0,-58} {1,-155}" -f $t.Name, $t.description}


$split = get-content C:\temp\services.txt
...
"Running" { $params.BackgroundColor = "Green" }
